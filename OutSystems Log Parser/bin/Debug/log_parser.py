...
replacementDict[1657] = ""
replacementDict[1630] = ""
replacementDict[1680] = ""
replacementDict[1839] = ""
replacementDict[1937] = ""
replacementDict[1945] = ""
replacementDict[1950] = ""
replacementDict[1962] = ""
replacementDict[1970] = ""
...
def prereqs(directoryPath, fromDate, toDate, createGraphs):
    ...
    if _fromDate > _toDate:
        ...
        if createGraphs[0].lower() == "y":
            ...
            scripts.iis.create_graph(os.getcwd() + "\\filtered_data_files\\iis_logs.txt", r"^([\d\-\:\. ]+)\|([\d\.]+)\|([\d]+)\|(?:(?:.*?\|){2})([\w\-\.\:\%)]+)\|([\w\-\.\:\%]+)\|([\d]+)\|(POST|PUT|PROPFIND|(?:n)?GET|OPTIONS|HEAD|ABCD|QUALYS|TRACE|SEARCH|RNDMMTD|TRACK|B(?:A)?DM(?:E)?T(?:H)?(?:O)?(?:D)?|CFYZ|DEBUG|MKCOL|INDEX|DELETE|PATCH|ACUNETIX)\|([\w\(\)\[\]\{\}\-\–\—\:\;\‘\’\'\"\“\”\,\.\<\>\«\»\`\~\á\Á\à\À\â\Â\ã\Ã\é\É\è\È\ê\Ê\í\Í\ì\Ì\î\Î\ó\Ó\ò\Ò\ô\Ô\õ\Õ\ú\Ú\ù\Ù\û\Û\ü\Ü\ñ\Ñ\ç\Ç\&\=\\\/\?\+\$\@\%\^\#\*\!\¿\¡\£\€\¢\¥\©\®]+)\|.+")
...
def logFile(absolutePath, filenameWithExt, ext, _fromDate, _toDate):
    iisNameRegex = re.match(iisFilenameRegex, filenameWithExt[:-4])
    if iisNameRegex:
        #IIS logs
        global numOfIISLogs
        numOfIISLogs+=1
        ...
        try:
            ...
            print("\nThe customer altered the original IIS logs.\n\nPossible reasons:\n1- The customer added a column on the logs.\n" +
                  "2- The customer switched the columns of the logs.\n")
    if not iisNameRegex:
        print("The IIS logs were not found\n\nPossible reasons:\n1- The customer renamed the file.\n" +
